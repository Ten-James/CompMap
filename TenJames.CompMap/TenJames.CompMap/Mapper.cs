...
public class BaseMapper : IMapper
{
    ...
    /// <returns></returns>
    protected virtual TDestination OnNull<TDestination>() => default;
    ...
    /// <typeparam name="TDestination">Destination type</typeparam>
    /// <returns></returns>
    protected virtual TDestination OnMap<TDestination>(object source, MethodInfo? mapToMethod,
        MethodInfo? mapFromMethod)
    {
        ...
        return OnError<TDestination>(source,
    ...
    protected virtual TDestination OnError<TDestination>(object source, Exception ex) => throw ex;
    ...
    protected virtual TDestination OnEnumerable<TDestination>(object source)
    {
        ...
        if (typeof(TDestination).IsArray)
        {
            ...
            {
                var mappedItem = GetType().GetMethod("Map").MakeGenericMethod(elementType)
        ...
        if (typeof(TDestination).IsGenericType
            && (typeof(TDestination).GetGenericTypeDefinition() == typeof(List<>)
        ...
        else
        {
            return OnError<TDestination>(source,
            new NotImplementedException(
            $"Mapping to collection type {typeof(TDestination).FullName} is not supported."));
        }
    }
